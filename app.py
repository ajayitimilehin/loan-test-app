...
import pandas as pd
import numpy as np
import plotly.express as px  # corrected alias
...
def loan_prediction(input_data):
    ...
    return "Sorry, your loan has not been approved" if prediction[0] == 0 else "Congratulations, you are eligible for the loan"

# Chart Page
def chart_page():
    st.title('Overview')

    # Creating a histogram for gender
    ...
    st.plotly_chart(gig_gender)  # Show the plot

    # Adding insights
    st.subheader('Insights')
    st.markdown("The chart shows that men tend to apply for loans more than women.")
...
def dashboard_page():
    ...
    col1, col2, col3 = st.columns(3)  # Specify the number of columns
    ...
    with col2:
        ...
        Property_Area_Rural = st.selectbox('Property_Area_Rural (Select 0 or 1)', options=[0, 1])
        Property_Area_Urban = st.selectbox('Property_Area_Urban (Select 0 or 1)', options=[0, 1])
    ...
    if st.button('Bank Loan Application'):
        try:
            ...
            st.success(result)  # fixed typo
        except ValueError:
            st.error('Enter a valid numeric value.')
